...
namespace OtelFinder.Repositories.Concrete
{
    public class ReservationRepository : BaseReservationRepository, IListOtel, IListReservation
    {
        ...
        public void ListOtelsLocation(ComboBox cmbLocation)
        {
            ...
            {
                cmbLocation.Items.Add(item.Location);

            }
        }

        public void ListOtelsName(ComboBox cmbOtelName,ComboBox cmbLocation)
        {
            foreach (var item in SeedData.otels)
            {
                if (Convert.ToString(item.Location) ==cmbLocation.Text)
                {
                    cmbOtelName.Items.Add(item.OtelName);
                }
            }
        }

        public void FillReservation(ComboBox cmbOtelName, Label lblOtelName,Label lblLocation,Label lblPrice)
        {
            foreach (var item in SeedData.otels)
            {
                if (item.OtelName==cmbOtelName.Text)
                {
